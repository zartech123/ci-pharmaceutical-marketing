...
class ScientificList extends CI_Controller {
	...
	{
		$this->load->view('scientificlist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->columns('action2','id_mer','nodoc','topic','event_venue','event_start_date','event_end_date','state','requested_by','action');
...
//			$crud->where('j2d7f2130.state=6 and type='.$_GET['type']);
    ...
    public function _callback_action($value, $row)
    {
		...
		$button = "";
		$jumlah = 0;
		$query = $this->db->query("SELECT id_sc from scientific_report WHERE state<7 and id_sc=".$row->id_sc);
		foreach ($query->result() as $row2)
		{
			$jumlah = $jumlah + 1;
		}
		...
		$query = $this->db->query("SELECT id_sc, id_mer, state from scientific WHERE id_sc=".$row->id_sc);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
			if($row2->state=="6" && $jumlah==0)
			{
				...
				{	
					$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ScientificReport?id_mer=".$row2->id_mer."&id_sc=".$row->id_sc."'><i class='fa fa-plus'></i>&nbsp;&nbsp;SERF Report</a>";
				}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		if(($this->session->userdata('id_group')==8 || $this->session->userdata('id_group')==9 || $this->session->userdata('id_group')==10) && $this->session->userdata('id_user')==$row->requested_by)
		{	
			$button="<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/Scientific?id_mer=".$row->id_mer."&id=".$row->id_sc."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}	
		else if($this->session->userdata('id_group')!=8 && $this->session->userdata('id_group')!=9 && $this->session->userdata('id_group')!=10)
		{
			$button="<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/Scientific?id_mer=".$row->id_mer."&id=".$row->id_sc."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		...
		$no = substr($row->created_date,0,4)."/SERF/".date("m",strtotime($row->created_date))."/".$row->nodoc;
		$url = base_url()."index.php/Scientific/delete?id=".$row->id_sc;

		if($row->state==1 && ($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12))
		{
//			$button=$button."<a type='button' id='".$name."' class='btn btn-danger btn-xs' target='popup' onclick=window.open('".base_url()."index.php/Scientific/delete?id=".$row->id_sc."','popup','width=300,height=100')><i class='fa fa-trash'></i>&nbsp;Delete</a><br>";
