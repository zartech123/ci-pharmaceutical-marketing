...
class ThankLetter2 extends CI_Controller {
...
//				if($max>0 && $max2>0)
	...
	public function add()
	{
		...
		{
			$this->db->insert("thank_letter2",$data);
			$id_tl = $this->db->insert_id();
		}
		else
		{
			$this->db->where('id_tl', $this->input->post('id_parent'));
			$this->db->update("thank_letter2",$data);
			$id_tl = $this->input->post('id_parent');
		}


		$query = $this->db->query("SELECT created_date FROM thank_letter2 where id_report=".$this->input->post('id_report'));
		foreach ($query->result() as $row2)
		{
			$created_date=$row2->created_date;
		}
...
//		Word
		...
		$section->addText(''.$this->input->post('event_institution'),'rStyle','Paragraph');
		...
		$section->addText('Merujuk pada '.$this->input->post('letter').' '.$this->input->post('event_institution').' tertanggal '.$letter_date2.', kami, PT Taisho Pharmaceutical Indonesia, Tbk. melalui surat ini hendak mengucapkan terima kasih atas kehadiran Anda sebagai perwakilan resmi yang ditunjuk '.$this->input->post('event_institution').' untuk menghadiri kegiatan ilmiah berikut:','rStyle2','Paragraph3');
...
//		$objWriter->save('C:\xampp\htdocs\taisho\assets\uploads\ThankYouLetterHCP'.$this->input->post('nodoc2').'.docx');
		$objWriter->save(APP_PATH.'assets/uploads/ThankYouLetter-HCO-'.date("Y",strtotime($created_date)).'-TL-'.date("m",strtotime($created_date)).'-'.$this->input->post('nodoc2').'.docx');



		redirect(base_url()."index.php/ThankLetter2?id_report=".$this->input->post('id_report')."&id=".$id_tl);
	}
