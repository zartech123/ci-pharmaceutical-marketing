...
class AgreementLetter2 extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['id_hcp']) == true) {
            ...
            if (isset($_GET['id']) == true) {
                $query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, id_hcp, nodoc2, event_title, event_description, fee, DATE_FORMAT(event_date,'%d/%m/%Y') as event_date, doctor, event_venue, doctor, event_start_time, event_end_time, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date FROM agreement_letter2 where id_tl=" . $_GET['id']);
            ...
            if ($k == 0) {
                ...
                $k = 0;
                $query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, payee, a.doctor, a.event_venue, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date FROM hcp a where id_hcp=" . $_GET['id_hcp']);
                foreach ($query->result() as $row2) {
                    $data = array(
                        'fee' => "",
                        'doctor' => $row2->doctor,
    ...
    public function add()
    {
        ...
        if (empty($this->input->post('id_parent'))) {
            $this->db->insert("agreement_letter2", $data);
            $id_tl = $this->db->insert_id();
        } else {
            $this->db->where('id_tl', $this->input->post('id_parent'));
            $this->db->update("agreement_letter2", $data);
            $id_tl = $this->input->post('id_parent');
        }

        $query = $this->db->query("SELECT created_date FROM agreement_letter2 where id_hcp=" . $this->input->post('id_hcp'));
        foreach ($query->result() as $row2) {
            $created_date = $row2->created_date;
        }
...
//        Word
        ...
        $PHPWord->addParagraphStyle('Paragraph5', array('align' => 'center', 'spaceAfter' => 200));
        ...
        $section->addText('', 'rStyle2', 'Paragraph3');
...
//        $objWriter->save('C:\xampp\htdocs\taisho\assets\uploads\AgreementLetterHCO'.$doctor2.'.docx');
        $objWriter->save(APP_PATH . 'assets/uploads/AgreementLetter-' . date("Y", strtotime($created_date)) . '-SA-' . date("m", strtotime($created_date)) . '-' . $this->input->post('nodoc2') . '.docx');

        redirect(base_url() . "index.php/AgreementLetter2?id_hcp=" . $this->input->post('id_hcp') . "&id=" . $id_tl);
    }
