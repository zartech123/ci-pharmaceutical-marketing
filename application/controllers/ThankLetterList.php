...
class ThankLetterList extends CI_Controller {
	...
	{
		$this->load->view('thank-letter-list',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp_report');
			$crud->set_subject('Thank You Letter HCP');
			...
			$crud->display_as('nodoc2','HCP Report');
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM hcp_report WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM thank_letter WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
    ...
    {
        $i = 0;
		$query = $this->db->query("SELECT id_report from thank_letter WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        }    
        if($i>0)
        {
			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ThankLetter?id_report=".$row->id_report."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Thank You Letter</a>";
		}
		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ThankLetter?id_report=".$row->id_report."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Thank You Letter</a>";
		}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		$query = $this->db->query("SELECT id_tl from thank_letter WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
			$name="tl".$row->id_report;
			$button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ThankLetter?id_report=".$row->id_report."&id=".$row2->id_tl."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}
