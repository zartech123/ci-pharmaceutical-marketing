...
class Channel5 extends CI_Controller {
	...
	{
		$this->load->view('channel5',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('user_branch');
			$crud->set_subject('Reporting (User Branch)');
			$crud->columns('id_user','id_dist','id_branch');
			$crud->fields('id_user','id_dist','id_branch');
			$crud->required_fields('id_branch','id_channel');
			...
			$crud->set_relation('id_user','user','name',' id_group in (10,15)',' id_group');
			$crud->set_relation('id_branch','branch','name','id_branch=0');
	...
	{
		$result="[";
		$query = $this->db->query("select id_branch, name from branch where id_dist='".$_GET['id']."' order by name");
		foreach ($query->result() as $row2)
		{			
