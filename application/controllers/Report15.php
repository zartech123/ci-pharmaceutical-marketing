...
class Report15 extends CI_Controller {
	...
	{
		$this->load->view('report15',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hco');
			$crud->set_subject('Outstanding Report HCO');
			$crud->columns('id_mer','nodoc2','event_name','event_organizer','event_venue','event_start_date','event_end_date','state','requested_by');
			$crud->set_relation('requested_by','user','name');
	//            $crud->set_relation('id_mer','mer','nodoc');
			$crud->where('state=6 and datediff(now(),updated_date2)>21 and id_hco not in (select id_hco from hco_report)');
