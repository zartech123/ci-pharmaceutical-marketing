...
class Channel3 extends CI_Controller {
	...
	{
		$this->load->view('channel3',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_subject('Internal Channel (Customer)');
			$crud->columns('id_cust','id_dist','id_channel','id_channel3');
			$crud->fields('id_cust','id_channel3');
			$crud->required_fields('id_channel3');
			$crud->unset_add();
			$crud->unset_delete();
			$crud->unset_read();
			$crud->unset_print();
			...
			$crud->set_relation('id_cust','customer','{name} - {id_cust2}');
			$crud->set_relation('id_channel','channel','name');
			$crud->set_relation('id_channel3','channel2','name');
			$crud->display_as('id_dist','Distributor');
			$crud->display_as('id_channel','Channel');
			$crud->display_as('id_cust','Customer');
			$crud->display_as('id_channel3','Internal Channel');
			$crud->callback_edit_field('id_cust', function ($value, $primary_key) 
			{
				$query = $this->db->query("SELECT name from customer where id_cust=".$value);
				$row = $query->row();
				return $row->name;
			});
