...
class Approver extends CI_Controller {
	...
	{
		$this->load->view('approver',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('approver');
			$crud->set_subject('Approver');
			$crud->columns('id_menu','prepared','approver1','approver2','approver3','approver4','approver5');
			$crud->required_fields('id_menu','prepared','approver1');
			$crud->fields('id_menu','prepared','approver1','approver2','approver3','approver4','approver5');
			if($this->session->userdata('id_group')=="11")
			{	
				$crud->set_relation('id_menu','menu_approver','name');
			}
			$crud->set_relation('prepared','groups','description');
			$crud->set_relation('approver1','groups','description');
			$crud->set_relation('approver2','groups','description');
			$crud->set_relation('approver3','groups','description');
			$crud->set_relation('approver4','groups','description');
			$crud->set_relation('approver5','groups','description');
/*			$group = array();
			$query = $this->db->query("SELECT id, description FROM groups order by description");
			foreach ($query->result() as $row2)
			{
//				$created_date=$row2->created_date;
				$group[$row2->id] = $row2->description;
			}*/
//			$crud->order_by('parent, order_m');
			$crud->unset_delete();
			$crud->unset_read();
			$crud->unset_print();
			$crud->unset_clone();
			$crud->display_as('id_menu','Menu');
			$crud->display_as('approver1','1st Approver');
			$crud->display_as('approver2','2nd Approver');
			$crud->display_as('approver3','3rd Approver');
			$crud->display_as('approver4','4th Approver');
			$crud->display_as('approver5','5th Approver');
