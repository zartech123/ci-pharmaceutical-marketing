...
class HCOReport extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_GET['id_mer'])==true && isset($_GET['id_hco'])==true)
		{				
			$k = 0;
			if(isset($_GET['id'])==true)
			{				
				$id_mer="";
				$query = $this->db->query("select title0, title1, approver0, approver1, prepared_note, active, id_mer, DATE_FORMAT(updated_date1,'%d-%M-%Y') as updated_date1, DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2, event_name, event_organizer, event_venue, requested_by, note1, state from hco_report where id_report=".$_GET['id']);
				foreach ($query->result() as $row2)
				{
					$id_mer = $row2->id_mer;
					$data = array(
			...
			if($k==0)
			{
				$query = $this->db->query("select max(nodoc2)+1 as nodoc from hco_report where year='".date("Y")."'");
				foreach ($query->result() as $row2)
				{
					if($row2->nodoc==NULL)
					{
				...
				$id_mer="";
				$query = $this->db->query("select id_mer, nodoc2, event_name, event_organizer, event_venue from hco where id_hco=".$_GET['id_hco']);
				foreach ($query->result() as $row2)
				{
					$id_mer = $row2->id_mer;
					$data = array(
				...
				$query = $this->db->query("select DATE_FORMAT(created_date,'%d-%M-%Y') as created_date from mer where id_mer=".$_GET['id_mer']);
				foreach ($query->result() as $row2)
				{
					$data = array_merge($data, array('created_date2'=>$row2->created_date));
				}
			...
			$data = array_merge($data, array('actual5'=>"0"));
			$data = array_merge($data, array('description5'=>""));
			if(isset($_GET['id'])==true)
			{
				$query = $this->db->query("select sponsor_type, description, budget, actual, kpk from budget_hco_report where id_parent=".$_GET['id']);
				foreach ($query->result() as $row2)
				{
					...
					else if($row2->sponsor_type=="Assosiation Fee")
					{
						...
						$data = array_merge($data, array('actual4'=>$row2->actual));
			...
			if($i==0)
			{
				$query = $this->db->query("select sponsor_type, description, local_amount, foreign_amount from budget_hco where id_parent=".$_GET['id_hco']);
				foreach ($query->result() as $row2)
				{
					$sponsor_text = "sponsor".($i+1);
					$budget_text = "budget".($i+1);
					...
					if($row2->sponsor_type=="Booth Stand")
					{
						...
						$data = array_merge($data, array('actual1'=>""));
						$data = array_merge($data, array('kpk1'=>"5"));
					}
					else if($row2->sponsor_type=="Symposia")
					{
						$data = array_merge($data, array('description2'=>$row2->description));
						$data = array_merge($data, array('budget2'=>$row2->local_amount));
						$data = array_merge($data, array('actual2'=>""));
						$data = array_merge($data, array('kpk2'=>"5"));
					}
					else if($row2->sponsor_type=="Institution Fee")
					{
						$data = array_merge($data, array('description3'=>$row2->description));
						$data = array_merge($data, array('budget3'=>$row2->local_amount));
						$data = array_merge($data, array('actual3'=>""));
						$data = array_merge($data, array('kpk3'=>"5"));
					}
					else if($row2->sponsor_type=="Assosiation Fee")
					{
						$data = array_merge($data, array('description4'=>$row2->description));
						$data = array_merge($data, array('budget4'=>$row2->local_amount));
						$data = array_merge($data, array('actual4'=>""));
						$data = array_merge($data, array('kpk4'=>"5"));
					}
					else
					{
						$data = array_merge($data, array($sponsor_text=>$row2->sponsor_type));
						...
						$data = array_merge($data, array($description_text=>$row2->description));
						$data = array_merge($data, array($actual_text=>""));
						$data = array_merge($data, array($kpk_text=>"5"));
					}
					$i = $i + 1;
			...
			{		
				if($jumlah==1)
				{
					$this->load->view('hco-report',$data);
				}
				else
				{
	...
	public function delete()
	{
		...
		$this->db->delete("budget_hco_report");

		$this->db->where('id_report', $_GET['id']);
		$this->db->delete("hco_report");
	...
	{
		$i = 0;
		$query = $this->db->query("select id_budget from budget_hco_report where id_parent=".$_GET['id']." order by id_budget");
		foreach ($query->result() as $row2)
		{
		...
		$this->db->update("budget_hco_report",$data3);
	...
	public function updateState()
	{
		if($this->session->userdata('id_user'))	$this->getAuditTrail();
		$state = 0;
		$query = $this->db->query("SELECT state FROM hco_report where id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$state = $row2->state;	
		}
		...
		$pd = "";
		$note1 = "";
		$query = $this->db->query("SELECT SUM(actual) AS amount FROM budget_hco_report where id_parent=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$amount = $row2->amount;	
		}
		...
		$query = $this->db->query("SELECT a.review, DATE_FORMAT(a.created_date,'%d-%M-%Y') as created_date, a.note1, a.state, DATE_FORMAT(event_start_date,'%d/%m/%Y') AS event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') AS event_end_date, a.nodoc2, a.event_name, a.event_organizer, name FROM hco_report a, user b, hco c WHERE c.id_hco=a.id_hco AND a.requested_by=b.id_user AND id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$note1 = $row2->note1;
		...
		if($state<2)
		{
			...
			$this->email->to($email);
			$this->email->cc($ma);
			$this->email->subject('Please Approve HCO Report with No '.$nodoc);
			...
			$content_html = $content_html."Please Click this link to <a href='".base_url()."index.php/HCOReport?id_mer=".$_GET['id_mer']."&id_hco=".$_GET['id_hco']."&id=".$_GET['id']."&access=".($_GET['id_group']-1)."'>Approve or Review or Reject</a>";
			$content_html = $content_html."<br><br>";
		...
		if($state>=2)
		{
			...
			$this->email->cc($ma);
			$this->email->subject('HCO Report with No '.$nodoc. ' has been Approved by '.$email3);
			$content_html = "<table style='border: 1px solid black;'><tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'><strong>Event Name</strong></td><td style='border: 1px solid black;'><strong>Event Organizer</strong></td><td style='border: 1px solid black;'><strong>Event Date</strong></td><td style='border: 1px solid black;'><strong>Cost Estimation</strong></td><td style='border: 1px solid black;'><strong>Created By</strong></td></tr>";
			$content_html = $content_html."<tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'>".$event_name."</td><td style='border: 1px solid black;'>".$event_organizer."</td><td style='border: 1px solid black;'>".$event_start_date." - ".$event_end_date."</td><td style='border: 1px solid black;'>".$amount."</td><td style='border: 1px solid black;'>".$created_by."</td></tr></table>";
			$content_html = $content_html."<br><br>";
			$this->email->message($content_html);			
	...
	public function updateState3()
	{
		...
		$query = $this->db->query("SELECT DATE_FORMAT(a.created_date,'%d-%M-%Y') as created_date, a.note1, a.state, DATE_FORMAT(event_start_date,'%d/%m/%Y') AS event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') AS event_end_date, a.nodoc2, a.event_name, a.event_organizer, name FROM hco_report a, user b, hco c WHERE c.id_hco=a.id_hco AND a.requested_by=b.id_user AND id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$note1 = $row2->note1;
			$nodoc = date("Y",strtotime($row2->created_date))."/R-HCO/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		...
		$data = array(
   				'state' => "7");
		$this->db->where('id_report', $_GET['id']);
		$this->db->update("hco_report",$data);

		if($state>=2)
		{
			$this->email->from('esponsorship@onetpi.co.id', 'Taisho E-Sponsorship Administrator');
			$this->email->to($email2);
			$this->email->subject('HCO Report with No '.$nodoc. ' has been Rejected by '.$email3);
	...
	public function updateState2()
	{
		...
		if($state>=2)
		{
			$this->email->from('esponsorship@onetpi.co.id', 'Taisho E-Sponsorship Administrator');
			$this->email->to($email2);
			$this->email->subject($email3.' Request to review HCO Report with No '.$nodoc);
			$content_html = "<table style='border: 1px solid black;'><tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'><strong>Event Name</strong></td><td style='border: 1px solid black;'><strong>Event Organizer</strong></td><td style='border: 1px solid black;'><strong>Event Date</strong></td><td style='border: 1px solid black;'><strong>Cost Estimation</strong></td><td style='border: 1px solid black;'><strong>Created By</strong></td><td style='border: 1px solid black;'><strong>Note</strong></td></tr>";
			$content_html = $content_html."<tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'>".$event_name."</td><td style='border: 1px solid black;'>".$event_organizer."</td><td style='border: 1px solid black;'>".$event_start_date." - ".$event_end_date."</td><td style='border: 1px solid black;'>".$amount."</td><td style='border: 1px solid black;'>".$created_by."</td><td style='border: 1px solid black;'>".$note."</td></tr></table>";
			$content_html = $content_html."<br><br>";
			$this->email->message($content_html);
	...
	public function add()
	{
		...
		{
			$this->db->insert("hco_report",$data);
		...
		else
		{
			...
			$query = $this->db->query("select id_budget from budget_hco_report where id_parent=".$id_hco." order by id_budget");
			foreach ($query->result() as $row2)
			{
				$id_budget[] = $row2->id_budget;
				$i = $i + 1;
		...
		if($i==0)
		{
			$this->db->insert("budget_hco_report",$data3);
		}
		else
		{
			$this->db->where('id_budget', $id_budget[0]);
		...
		$data3 = array(
            'sponsor_type' => "Association Fee",
            'budget' => $this->input->post('budget4'),
            'actual' => $this->input->post('actual4'),
            'id_parent' => $id_hco,
            'kpk' => $this->input->post('kpk4'),
		...
		redirect(base_url()."index.php/HCOReport?id_mer=".$this->input->post('id_mer')."&id=".$id_hco."&id_hco=".$this->input->post('id_hco'));
	...
	{
		$type = 0;
		$query = $this->db->query("select distinct file_type from attachment where type=7 and id_parent=".$_GET['id']." and file_type in (2) order by file_type");
		foreach ($query->result() as $row2)
		{
	...
	public function getAttachment()
	{
		...
		$jumlah = 0;
		$query = $this->db->query("SELECT COUNT(*) AS jumlah FROM budget_hco_report WHERE sponsor_type='Symposia' AND actual>0 AND id_parent=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$jumlah = $row2->jumlah;	
		}
		...
		$result = "";
		$file_type = array("1"=>"Others","2"=>"Photo","3"=>"Attendance List");
		$query = $this->db->query("select id_attachment, file_name, file_type from attachment where type=7 and id_parent=".$_GET['id']." order by file_type");
		foreach ($query->result() as $row2)
		{	
