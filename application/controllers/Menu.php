...
class Menu extends CI_Controller {
	...
	{
		$this->load->view('menu',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('menu_group');
			$crud->set_subject('Menu');
			$crud->columns('id_menu','id_group');
			$crud->fields('id_menu','id_group');
			if($this->session->userdata('id_group')=="11")
			{	
				$crud->where('type in (0,3)');
				$crud->set_relation('id_menu','menu','name','page<>"" and is_all=0 and type in (0, 3)','parent, menu_order');
			}
			else if($this->session->userdata('id_group')=="13")
			{
				$crud->where('type in (1,3)');
				$crud->set_relation('id_menu','menu','name','page<>"" and is_all=0 and type in (1,3)','parent, menu_order');
			}				
//			$crud->set_relation('id_group','groups','description');
			$group = array();
...
//			$crud->order_by('parent, order_m');
			...
			$crud->field_type('id_group','multiselect',$group);
