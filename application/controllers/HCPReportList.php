...
class HCPReportList extends CI_Controller {
	...
	{
		$this->load->view('hcp-reportlist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_subject('HCP POST EVENT REPORT');
            $crud->columns('action2','id_mer','id_hcp2','id_report','event_name','event_organizer','event_institution','event_start_date','event_end_date','state','requested_by');
...
//			$crud->where('j1cee65f4.state=6 and type='.$_GET['type']);
			...
			$crud->display_as('id_hcp2','HCP SRF 2');
			$crud->display_as('id_report','HCP Report');
            ...
            $crud->callback_column('id_hcp2',array($this,'_callback_nodoc3'));
	...
	public function _callback_nodoc2($value, $row)
    {
		...
		foreach ($query->result() as $row2)
		{			
			$result = substr($row2->created_date,-4)."/R-HCP/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		}
    ...
    public function _callback_action2($value, $row)
    {
		...
		{	
			$button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCPReport?id_mer=".$row->id_mer."&id=".$row->id_report."&id_hcp2=".$row->id_hcp2."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}	
		...
		$no = substr($row->created_date,0,4)."/R-HCP/".date("m",strtotime($row->created_date))."/".$row->nodoc2;
		$url = base_url()."index.php/HCPReport/delete?id=".$row->id_report;
