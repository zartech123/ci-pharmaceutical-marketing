...
class AgreementLetterList2 extends CI_Controller {
	...
	{
		$this->load->view('agreement-letter-list2',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp');
			$crud->set_subject('SPEAKER AGREEMENT LETTER - HCP Consultant');
            $crud->columns('action2','id_mer','nodoc2','nodoc','event_name','doctor','event_organizer','event_venue','event_start_date','event_end_date','action');
...
//			$id_hcp = $id_hcp."0)";
			...
			$crud->display_as('nodoc','Speaker Agreement');
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM agreement_letter2 WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
    ...
    {
        $i = 0;
		$query = $this->db->query("SELECT id_hcp from agreement_letter2 WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        }    
        if($i>0)
        {
			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/AgreementLetter2?id_hcp=".$row->id_hcp."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Speaker Agreement</a>";
		}
		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/AgreementLetter2?id_hcp=".$row->id_hcp."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Speaker Agreement</a>";
		}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		$query = $this->db->query("SELECT id_tl from agreement_letter2 WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
			$name="tl".$row->id_hcp;
			$button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/AgreementLetter2?id_hcp=".$row->id_hcp."&id=".$row2->id_tl."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}
