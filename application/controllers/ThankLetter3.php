...
class ThankLetter3 extends CI_Controller {
	...
	public function index()
	{
		...
		{				
			...
			{				
				$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, id_report, nodoc2, letter, DATE_FORMAT(letter_date,'%d/%m/%Y') as letter_date, doctor, event_name, event_venue, event_institution, doctor, event_address, event_start_time, event_end_time, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date FROM thank_letter3 where id_tl=".$_GET['id']);
				...
				/*$query = $this->db->query("select DATE_FORMAT(created_date,'%d-%M-%Y') as created_date from scientific_hcp where id_sc_hcp=".$id_report);
				foreach ($query->result() as $row2)
				{
...
//				if($max>0 && $max2>0)
				...
				$query = $this->db->query("SELECT hcp, DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, event_name, event_venue, event_institution, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date FROM hcp_report2 where id_report=".$_GET['id_report']);
				foreach ($query->result() as $row2)
				{
					$data = array(
						'event_institution' => $row2->event_institution,
				...
				{		
					if($jumlah==1)
					{
						$this->load->view('thank-letter3',$data);
					}
					else
					{
	...
	public function add()
	{
		...
		$query = $this->db->query("SELECT created_date FROM thank_letter3 where id_report=".$this->input->post('id_report'));
		foreach ($query->result() as $row2)
		{
			$created_date=$row2->created_date;
		}
...
//		Word
		...
		$query = $this->db->query("SELECT actual, sponsor_type FROM budget_hcp_report2 where actual>0 and id_parent=".$this->input->post('id_report'));
		foreach ($query->result() as $row2)
		{
...
//		$objWriter->save('C:\xampp\htdocs\taisho\assets\uploads\ThankYouLetterScientific'.$this->input->post('nodoc2').'.docx');
		$objWriter->save(APP_PATH.'assets/uploads/ThankYouLetter-HCPC-'.date("Y",strtotime($created_date)).'-TL-'.date("m",strtotime($created_date)).'-'.$this->input->post('nodoc2').'.docx');

		redirect(base_url()."index.php/ThankLetter3?id_report=".$this->input->post('id_report')."&id=".$id_tl);
	}
