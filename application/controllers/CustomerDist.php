...
class CustomerDist extends CI_Controller {
	...
	{
		$this->load->view('customer_dist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('customer_redistribution');
			$crud->set_subject('Customer Redistributor');
			$crud->columns('id_dist','id_cust','id_branch');
			$crud->fields('id_dist','id_cust','id_branch');
			$crud->set_relation('id_dist','distributor','code');
			$crud->set_relation('id_branch','branch','name',array('id_dist' => '2'));
			$crud->set_relation('id_cust','customer','{name} - {id_cust2}','id_cust=0');
			if($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12 || $this->session->userdata('id_group')==13)
			{
			}				
			else
			{
	...
	{
		$result="[";
		$query = $this->db->query("SELECT distinct id_cust as id, name  FROM customer WHERE replace(name,' ','') like '%".$_GET['keyword']."%' and id_dist=2");
		foreach ($query->result() as $row2)
		{			
			$result=$result."{\"id\":\"".$row2->id."\",\"name\":\"".str_replace('"', '',$row2->name)."\"}";
			$result=$result.",";
		}
		$result=rtrim($result, ",");
		$result=$result."]";	
