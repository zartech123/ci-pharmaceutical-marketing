...
class HCP2List extends CI_Controller {
	...
	{
		$this->load->view('hcp2list',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp2');
			$crud->set_subject('HCP SPONSORSHIP REQUEST FORM 2');
            $crud->columns('action2','id_mer','nodoc','nodoc2','event_name','event_organizer','event_venue','event_institution','doctor','event_start_date','event_end_date','state','requested_by','action');
...
//			$crud->where('j2d7f2130.state=6 and type='.$_GET['type']);
    ...
    {
		$i = 0;
		$query = $this->db->query("SELECT id_report from hcp_report WHERE state<7 and id_hcp2=".$row->id_hcp2);
		$button = "";
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        ...
        if($i==0 && $row->state=="Approved")
        {
			...
			$query = $this->db->query("SELECT id_mer from hcp2 WHERE id_hcp2=".$row->id_hcp2);
			foreach ($query->result() as $row2)
			{			
				...
				{	
					$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCPReport?id_mer=".$row2->id_mer."&id_hcp2=".$row->id_hcp2."'><i class='fa fa-plus'></i>&nbsp;&nbsp;HCP Report</a>";
				}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		if(($this->session->userdata('id_group')==8 || $this->session->userdata('id_group')==9 || $this->session->userdata('id_group')==10) && $this->session->userdata('id_user')==$row->requested_by)
		{	
			$button="<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCP2?id_mer=".$row->id_mer."&id=".$row->id_hcp2."&id_hcp1=".$row->id_hcp1."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}
		else if($this->session->userdata('id_group')!=8 && $this->session->userdata('id_group')!=9 && $this->session->userdata('id_group')!=10)
		{
			$button="<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCP2?id_mer=".$row->id_mer."&id=".$row->id_hcp2."&id_hcp1=".$row->id_hcp1."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}			
		$no = substr($row->created_date,0,4)."/HCP2/".date("m",strtotime($row->created_date))."/".$row->nodoc2;
		$url = base_url()."index.php/HCP2/delete?id=".$row->id_hcp2;
