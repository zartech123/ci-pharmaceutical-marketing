...
class ConfirmationLetterHCPList extends CI_Controller {
	...
	{
		$this->load->view('confirmation-letter-hcplist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp2');
			$crud->set_subject('SPONSORSHIP CONFIRMATION LETTER HCP');
            $crud->columns('action2','id_mer','nodoc2','nodoc','event_name','event_organizer','event_venue','event_institution','doctor','event_start_date','event_end_date','action');
...
//            $crud->set_relation('id_mer','mer','nodoc');
			...
			$crud->display_as('nodoc2','HCP SRF2');
			$crud->display_as('nodoc','Confirmation Letter');
    ...
    {
        $i = 0;
		$query = $this->db->query("SELECT id_cl from confirmation_letter_hcp WHERE id_hcp2=".$row->id_hcp2);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        }    
//        if($i>0)
        {
//			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCP?id_hcp2=".$row->id_hcp2."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Confirmation Letter</a>";
		}
//		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCP?id_hcp2=".$row->id_hcp2."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Confirmation Letter</a>";
		}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		$query = $this->db->query("SELECT id_cl, concat(event_institution,'-',name) as doctor from confirmation_letter_hcp a, doctor b WHERE a.doctor=b.id_doctor and id_hcp2=".$row->id_hcp2);
		foreach ($query->result() as $row2)
		{			
            $name="cl".$row2->id_cl;
            $button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCP?id_hcp2=".$row->id_hcp2."&id=".$row2->id_cl."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM confirmation_letter_hcp WHERE id_hcp2=".$row->id_hcp2);
		foreach ($query->result() as $row2)
		{			
			$result = substr($row2->created_date,-4)."/HCP2/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		}
