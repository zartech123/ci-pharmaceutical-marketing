...
class HCPReport extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_GET['id_mer'])==true && isset($_GET['id_hcp2'])==true)
		{				
			$k = 0;
			if(isset($_GET['id'])==true)
			{				
				$id_mer="";
				$query = $this->db->query("select title0, title1, approver0, approver1, prepared_note, active, id_mer, DATE_FORMAT(updated_date1,'%d-%M-%Y') as updated_date1, DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, event_name, event_organizer, event_venue, nodoc2, requested_by, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date, event_institution, event_contact, event_address, hcp, speciality, note1, state from hcp_report where id_report=".$_GET['id']);
				foreach ($query->result() as $row2)
				{
					$id_mer = $row2->id_mer;
					$data = array(
			...
			if($k==0)
			{
				$query = $this->db->query("select max(nodoc2)+1 as nodoc from hcp_report where year='".date("Y")."'");
				foreach ($query->result() as $row2)
				{
					if($row2->nodoc==NULL)
					{
				...
				$id_hcp1="";
				$id_mer="";
				$query = $this->db->query("select id_mer, name, id_hcp1, event_name, event_organizer, event_venue, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date, event_institution, event_contact, a.doctor from hcp2 a, doctor b where a.doctor=b.id_doctor and id_hcp2=".$_GET['id_hcp2']);
				foreach ($query->result() as $row2)
				{
				...
				$query = $this->db->query("select speciality, hcp from hcp1 where id_hcp1=".$id_hcp1);
				foreach ($query->result() as $row2)
				{
					$data = array_merge($data, array('speciality'=>$row2->speciality));
				}
			}
			...
			if(isset($_GET['id'])==true)
			{
				$query = $this->db->query("select sponsor_type, description, budget, actual, kpk from budget_hcp_report where id_parent=".$_GET['id']);
				foreach ($query->result() as $row2)
				{
			...
			if($i==0)
			{
				...
				$query = $this->db->query("select sponsor_type, description, local_amount, foreign_amount from budget_hcp2 where id_parent=".$_GET['id_hcp2']);
				foreach ($query->result() as $row2)
				{
					$sponsor_text = "sponsor".($i+1);
					$budget_text = "budget".($i+1);
					...
					if($row2->sponsor_type=="Registration Fee")
					{
						...
						$data = array_merge($data, array('kpk1'=>"3"));
					}
					else if($row2->sponsor_type=="Travel")
					{
						$data = array_merge($data, array('description1'=>$row2->description));
						...
						$data = array_merge($data, array('kpk2'=>"2"));
					}
					else if($row2->sponsor_type=="Accommodation")
					{
						$data = array_merge($data, array('description2'=>$row2->description));
						...
						$data = array_merge($data, array('kpk3'=>"1"));
					}
					else
					{
						$data = array_merge($data, array($sponsor_text=>$row2->sponsor_type));
			...
			{		
				if($jumlah==1)
				{
					$this->load->view('hcp-report',$data);
				}
				else
				{
	...
	public function delete()
	{
		...
		$this->db->delete("budget_hcp_report");

		$this->db->where('id_report', $_GET['id']);
		$this->db->delete("hcp_report");
	...
	public function updateState()
	{
		if($this->session->userdata('id_user'))	$this->getAuditTrail();
		$state = 0;
		$query = $this->db->query("SELECT state FROM hcp_report where id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$state = $row2->state;	
		}
		...
		$pd = "";
		$note1 = "";
		$query = $this->db->query("SELECT sum(replace(actual,'.','')) AS amount FROM budget_hcp_report where id_parent=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$amount = $row2->amount;	
		}
		...
		$query = $this->db->query("SELECT review, DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, note1, state, DATE_FORMAT(event_start_date,'%d/%m/%Y') AS event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') AS event_end_date, nodoc2, event_name, event_organizer, name FROM hcp_report a, user b WHERE a.requested_by=b.id_user AND id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$note1 = $row2->note1;
		...
		if($state<2)
		{
			...
			$this->email->to($email);
			$this->email->cc($ma);
			$this->email->subject('Please Approved HCP Report with No '.$nodoc);
			...
			$content_html = $content_html."Please Click this link to <a href='".base_url()."index.php/HCPReport?id_mer=".$_GET['id_mer']."&id_hcp2=".$_GET['id_hcp2']."&id=".$_GET['id']."&access=".($_GET['id_group']-1)."'>Approve or Review or Reject</a>";
			$content_html = $content_html."<br><br>";
		...
		if($state>=2)
		{
			...
			$this->email->cc($ma);
			$this->email->subject('HCP Report with No '.$nodoc. ' has been Approved by '.$email3);
			$content_html = "<table style='border: 1px solid black;'><tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'><strong>Event Name</strong></td><td style='border: 1px solid black;'><strong>Event Organizer</strong></td><td style='border: 1px solid black;'><strong>Event Date</strong></td><td style='border: 1px solid black;'><strong>Cost Estimation</strong></td><td style='border: 1px solid black;'><strong>Created By</strong></td></tr>";
			$content_html = $content_html."<tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'>".$event_name."</td><td style='border: 1px solid black;'>".$event_organizer."</td><td style='border: 1px solid black;'>".$event_start_date." - ".$event_end_date."</td><td style='border: 1px solid black;'>".$amount."</td><td style='border: 1px solid black;'>".$created_by."</td></tr></table>";
			$content_html = $content_html."<br><br>";
			$this->email->message($content_html);			
	...
	public function updateState3()
	{
		...
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, note1, state, DATE_FORMAT(event_start_date,'%d/%m/%Y') AS event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') AS event_end_date, nodoc2, event_name, event_organizer, name FROM hcp_report a, user b WHERE a.requested_by=b.id_user AND id_report=".$_GET['id']);
		foreach ($query->result() as $row2)
		{
			$note1 = $row2->note1;
			$nodoc = date("Y",strtotime($row2->created_date))."/R-HCP/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		...
		$data = array(
   				'state' => "7");
		$this->db->where('id_report', $_GET['id']);
		$this->db->update("hcp_report",$data);

		if($state>=2)
		{
			$this->email->from('esponsorship@onetpi.co.id', 'Taisho E-Sponsorship Administrator');
			$this->email->to($email2);
			$this->email->subject('HCP Report with No '.$nodoc. ' has been Rejected by '.$email3);
	...
	{
		$i = 0;
		$query = $this->db->query("select id_budget from budget_hcp_report where id_parent=".$_GET['id']." order by id_budget");
		foreach ($query->result() as $row2)
		{
		...
		$this->db->update("budget_hcp_report",$data3);
	...
	public function updateState2()
	{
		...
		if($state>=2)
		{
			$this->email->from('esponsorship@onetpi.co.id', 'Taisho E-Sponsorship Administrator');
			$this->email->to($email2);
			$this->email->subject($email3.' Request to review HCP Report with No '.$nodoc);
			$content_html = "<table style='border: 1px solid black;'><tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'><strong>Event Name</strong></td><td style='border: 1px solid black;'><strong>Event Organizer</strong></td><td style='border: 1px solid black;'><strong>Event Date</strong></td><td style='border: 1px solid black;'><strong>Cost Estimation</strong></td><td style='border: 1px solid black;'><strong>Created By</strong></td><td style='border: 1px solid black;'><strong>Note</strong></td></tr>";
			$content_html = $content_html."<tr style='text-align:center;border: 1px solid black;'><td style='border: 1px solid black;'>".$event_name."</td><td style='border: 1px solid black;'>".$event_organizer."</td><td style='border: 1px solid black;'>".$event_start_date." - ".$event_end_date."</td><td style='border: 1px solid black;'>".$amount."</td><td style='border: 1px solid black;'>".$created_by."</td><td style='border: 1px solid black;'>".$note."</td></tr></table>";
			$content_html = $content_html."<br><br>";
			$this->email->message($content_html);
	...
	public function add()
	{
		...
		{
			$this->db->insert("hcp_report",$data);
		...
		else
		{
			...
			$query = $this->db->query("select id_budget from budget_hcp_report where id_parent=".$id_hcp." order by id_budget");
			foreach ($query->result() as $row2)
			{
				$id_budget[] = $row2->id_budget;
				$i = $i + 1;
		...
		if($i==0)
		{
			$this->db->insert("budget_hcp_report",$data3);
		}
		else
		{
			$this->db->where('id_budget', $id_budget[0]);
...
//		echo "<script>window.close();</script>";
		...
		redirect(base_url()."index.php/HCPReport?id_mer=".$this->input->post('id_mer')."&id=".$id_hcp."&id_hcp2=".$this->input->post('id_hcp2'));
	...
	{
		$type = 0;
		$query = $this->db->query("select distinct file_type from attachment where type=10 and id_parent=".$_GET['id']." and file_type in (2) order by file_type");
		foreach ($query->result() as $row2)
		{
	...
	public function getAttachment()
	{
		...
		$query = $this->db->query("select id_attachment, file_name, file_type from attachment where type=10 and id_parent=".$_GET['id']." order by file_type");
		foreach ($query->result() as $row2)
		{	
