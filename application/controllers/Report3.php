...
//		for($k=1;$k<=7;$k++)
	...
	public function getTotal()
	{
		...
		{
			$id_cust="";
			$query2 = $this->db->query("select distinct id_cust from customer_redistribution where id_dist in (".$value.")");
			...
			$id_cust2="";
			$query2 = $this->db->query("select distinct id_cust from channel a, customer b where id_cust not in (".$id_cust.") and a.id_channel=b.id_channel and b.id_dist in (".$value.") and a.big in (".$_GET['id_channel'].")");
			foreach ($query2->result() as $row3)
			{			
		...
		for($j=$_GET['year1'];$j<=$_GET['year2'];$j++)
		{
			...
			foreach ($id_dist as &$value)
			{
				...
				{
						$divider=$_GET['month2']-$_GET['month1']+1;
						$query = $this->db->query("select count(distinct id_cust) as total from invoice_sum where sales_value>0 and id_product in (".$_GET['id_product'].") and id_cust in (".$id_custarray[$value].") and substr(period,1,4)='".$j."' and period between '".$j.str_pad($_GET['month2'],2,"0",STR_PAD_LEFT)."' and '".$j.str_pad($_GET['month1'],2,"0",STR_PAD_LEFT)."'");
				}
				else
				{	
					if($j==$_GET['year1'])
	...
	public function getTotal3()
	{
		...
		foreach ($id_dist as &$value) 
		{
			...
			$id_cust2="";
			$query2 = $this->db->query("select distinct id_cust from channel a, customer b where is_pharmachain in (".$pharma.") and id_cust not in (".$id_cust.") and a.id_channel=b.id_channel and b.id_dist in (".$value.") and a.big in (".$_GET['id_channel'].")");
			foreach ($query2->result() as $row3)
			{			
				$id_cust2=$id_cust2."'".$row3->id_cust."',";				
			}
	...
	//			$id_cust=rtrim($id_cust,",");
				...
				if($_GET['year2']==$_GET['year1'])
				{
						for($j=$_GET['month1'];$j<=$_GET['month2'];$j++)
						{
							...
							$query = $this->db->query("select count(distinct id_cust) as total from invoice_sum where sales_value>0 and id_product in (".$_GET['id_product'].") and id_cust in (".$id_cust2.") and period='".$i.str_pad($j,2,"0",STR_PAD_LEFT)."'");
							foreach ($query->result() as $row2)
							{			
								if($row2->total==null)
								{
	...
	public function getTotal2()
	{
		...
		foreach ($id_dist as &$value) 
		{
			...
			$result2[$value-1] = 0;
			$result4[$value-1] = 0;
			$query = $this->db->query("select count(distinct id_cust)/".$_GET['month2']." as total from invoice_sum where sales_value>0 and id_product in (".$_GET['id_product'].") and id_cust in (".$id_cust2.") and period between '".$_GET['year2']."01' and '".$_GET['year2'].str_pad($_GET['month2'],2,"0",STR_PAD_LEFT)."'");
			foreach ($query->result() as $row2)
			{
				if($row2->total==null)
				{
			...
			$query = $this->db->query("select count(distinct id_cust)/".(12-$_GET['month1']+1)." as total from invoice_sum where sales_value>0 and id_product in (".$_GET['id_product'].") and id_cust in (".$id_cust2.") and period between '".$_GET['year1'].str_pad($_GET['month1'],2,"0",STR_PAD_LEFT)."' and '".$_GET['year1']."12'");
			foreach ($query->result() as $row2)
			{
