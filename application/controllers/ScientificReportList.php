...
class ScientificReportList extends CI_Controller {
	...
	{
		$this->load->view('scientific-reportlist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('scientific_report');
			$crud->set_subject('SCIENTIFIC POST EVENT REPORT');
            $crud->columns('action2','id_mer','id_sc','id_report','topic','event_venue','event_start_date','event_end_date','state','requested_by');
...
//            $crud->where('j2d7f2130.state=6 and type='.$_GET['type']);
			...
			$crud->display_as('id_sc','SERF');
			$crud->display_as('id_report','SERF Report');
...
//            $crud->callback_column('action',array($this,'_callback_action'));
            ...
            $crud->callback_column('id_sc',array($this,'_callback_nodoc3'));
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM scientific_report WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
			$result = date("Y",strtotime($row2->created_date))."/R-SERF/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		}
    ...
    public function _callback_nodoc3($value, $row)
    {
		...
		foreach ($query->result() as $row2)
		{			
			$result = date("Y",strtotime($row2->created_date))."/SERF/".date("m",strtotime($row2->created_date))."/".$row2->nodoc;
		}
    ...
    public function _callback_action2($value, $row)
    {
		...
		{	
			$button="<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ScientificReport?id_mer=".$row->id_mer."&id=".$row->id_report."&id_sc=".$row->id_sc."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}	
		...
		$no = substr($row->created_date,0,4)."/R-SERF/".date("m",strtotime($row->created_date))."/".$row->nodoc2;
		$url = base_url()."index.php/ScientificReport/delete?id=".$row->id_report;

		if($row->state==1 && ($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12))
		{
//			$button=$button."<a type='button' id='".$name."' class='btn btn-danger btn-xs' target='popup' onclick=window.open('".base_url()."index.php/ScientificReport/delete?id=".$row->id_report."','popup','width=300,height=100')><i class='fa fa-trash'></i>&nbsp;Delete</a><br>";
