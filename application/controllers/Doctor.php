...
class Doctor extends CI_Controller {
	...
	{
		$this->load->view('doctor',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->columns('id_speciality','name');
			$crud->set_relation('id_speciality','speciality','name_speciality');
			if($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12 || $this->session->userdata('id_group')==6)
			{
			}				
			else
			{
			...
			$crud->unset_print();
			$crud->unset_clone();
			$crud->display_as('id_speciality','Speciality');
			$crud->display_as('name','Doctor');
