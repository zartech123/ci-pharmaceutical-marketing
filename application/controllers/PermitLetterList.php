...
class PermitLetterList extends CI_Controller {
	...
	{
		$this->load->view('permit-list',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp');
			$crud->set_subject('PERMIT LETTER');
            $crud->columns('action2','id_mer','nodoc2','nodoc','event_name','event_organizer','event_venue','event_start_date','event_end_date','action');
//            $crud->set_relation('doctor','doctor','name');
//			$crud->set_relation('id_mer','mer','nodoc');
//			$id_hcp ="(";
//			$query = $this->db->query("SELECT id_parent from budget_hcp WHERE sponsor_type='Symposia' and (local_amount>0 or foreign_amount>0)");
//			foreach ($query->result() as $row2)
//			{			
//				$id_hcp = $id_hcp.$row2->id_parent.",";
//			}    
//			$id_hcp = $id_hcp."0)";
            $crud->order_by('year, nodoc2');
			$crud->where('hcp.state=6');
			$crud->unset_add();
			$crud->unset_edit();
			$crud->unset_delete();
			$crud->unset_read();
			...
			$crud->display_as('nodoc','Permit Letter');
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM permit_letter WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
			$result = substr($row2->created_date,-4)."/PL/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		}
    ...
    {
        $i = 0;
		$query = $this->db->query("SELECT id_hcp from permit_letter WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        }    
        if($i>0)
        {
			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/PermitLetter?id_hcp=".$row->id_hcp."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Permit Letter</a>";
		}
		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/PermitLetter?id_hcp=".$row->id_hcp."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Permit Letter</a>";
		}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		$query = $this->db->query("SELECT distinct id_tl from permit_letter WHERE id_hcp=".$row->id_hcp);
		foreach ($query->result() as $row2)
		{			
			$name="tl".$row->id_hcp;
			$button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/PermitLetter?id_hcp=".$row->id_hcp."&id=".$row2->id_tl."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";
		}
