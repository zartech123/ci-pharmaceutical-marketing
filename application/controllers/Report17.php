...
class Report17 extends CI_Controller {
	...
	{
		$this->load->view('report17',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hcp');
			$crud->set_subject('Outstanding Report HCP CONSULTANT');
            $crud->columns('id_mer','nodoc2','event_name','event_organizer','event_venue','event_institution','doctor','event_start_date','event_end_date','state','requested_by');
			...
			$crud->where('state=6 and datediff(now(),updated_date2)>21 and id_hcp not in (select id_hcp from hcp)');
