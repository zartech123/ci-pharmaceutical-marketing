...
class ThankLetterList2 extends CI_Controller {
...
//			$crud->callback_column('event_end_date',array($this,'callback_date'));
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM thank_letter2 WHERE id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
    ...
    public function _callback_action($value, $row)
    {
        ...
        if($i>0)
        {
			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ThankLetter2?id_report=".$row->id_report."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Thank You Letter</a>";
		}
		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ThankLetter2?id_report=".$row->id_report."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Thank You Letter</a>";
		}	
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(event_start_date,'%d-%M-%Y') as event_start_date FROM hco_report a, hco b WHERE a.id_hco=b.id_hco and id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(event_end_date,'%d-%M-%Y') as event_end_date FROM hco_report a, hco b WHERE a.id_hco=b.id_hco and id_report=".$row->id_report);
		foreach ($query->result() as $row2)
		{			
