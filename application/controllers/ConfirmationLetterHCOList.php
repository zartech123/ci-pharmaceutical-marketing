...
class ConfirmationLetterHCOList extends CI_Controller {
	...
	{
		$this->load->view('confirmation-letter-hcolist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hco');
			$crud->set_subject('SPONSORSHIP CONFIRMATION LETTER HCO');
...
//            $crud->set_relation('id_mer','mer','nodoc');
    ...
    {
		$result="";
		$query = $this->db->query("SELECT DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, nodoc2 FROM confirmation_letter_hco WHERE id_hco=".$row->id_hco);
		foreach ($query->result() as $row2)
		{			
			$result = substr($row2->created_date,-4)."/HCO/".date("m",strtotime($row2->created_date))."/".$row2->nodoc2;
		}
    ...
    {
        $i = 0;
		$query = $this->db->query("SELECT id_cl from confirmation_letter_hco WHERE id_hco=".$row->id_hco);
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
        }    
        if($i>0)
        {
			$button="<a type='button' id='create' style='visibility:hidden' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCO?id_hco=".$row->id_hco."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Confirmation Letter</a>";
		}
		else
		{
			$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCO?id_hco=".$row->id_hco."'><i class='fa fa-plus'></i>&nbsp;&nbsp;Confirmation Letter</a>";
		}	
    ...
    public function _callback_action2($value, $row)
    {
		...
		$query = $this->db->query("SELECT id_cl, event_name from confirmation_letter_hco WHERE id_hco=".$row->id_hco);
		foreach ($query->result() as $row2)
		{			
            $name="cl".$row2->id_cl;
            $button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/ConfirmationLetterHCO?id_hco=".$row->id_hco."&id=".$row2->id_cl."'>&nbsp;Edit</a>&nbsp;";
		}
