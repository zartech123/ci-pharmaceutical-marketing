...
class Channel4 extends CI_Controller {
	...
	{
		$this->load->view('channel4',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('channel_group');
			$crud->set_subject('Internal Channel (User Group)');
			$crud->columns('id_group','id_channel');
			$crud->fields('id_group','id_channel');
			$crud->required_fields('id_group','id_channel');
			...
			$crud->unset_print();
			$crud->unset_clone();
			$crud->set_relation('id_channel','channel2','name');
			$crud->set_relation('id_group','groups','description',' id in (2,3,4,6,7,13)','id asc');
			$crud->display_as('id_channel','Internal Channel');
