...
class HCOList extends CI_Controller {
	...
	{
		$this->load->view('hcolist',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->set_table('hco');
			$crud->set_subject('HCO SPONSORSHIP REQUEST FORM');
            $crud->columns('action2','id_mer','nodoc2','event_name','event_organizer','event_venue','event_start_date','event_end_date','state','requested_by','action');
...
//			$crud->where('j2d7f2130.state=6 and type='.$_GET['type']);
    ...
    {
		$i = 0;
		$query = $this->db->query("SELECT id_report from hco_report WHERE state<7 and id_hco=".$row->id_hco);
		$button = "";
		foreach ($query->result() as $row2)
		{			
            $i = $i+1;
		...
		{
			$query = $this->db->query("SELECT id_mer from hco WHERE id_hco=".$row->id_hco);
			foreach ($query->result() as $row2)
			{			
				$button="<a type='button' id='create' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCOReport?id_mer=".$row2->id_mer."&id_hco=".$row->id_hco."'><i class='fa fa-plus'></i>&nbsp;&nbsp;HCO Report</a>";
			}	
		}
		else
		{
	...
	public function _callback_action2($value, $row)
    {
        ...
        $name="hco".$row->id_hco;
        $button=$button."<a type='button' id='".$name."' class='btn btn-default btn-xs' target='_blank' href='".base_url()."index.php/HCO?id_mer=".$row->id_mer."&id=".$row->id_hco."'><i class='fa fa-pencil'></i>&nbsp;Edit</a>&nbsp;";

		$no = substr($row->created_date,0,4)."/HCO/".date("m",strtotime($row->created_date))."/".$row->nodoc2;
		$url = base_url()."index.php/HCO/delete?id=".$row->id_hco;

		if($row->state==1 && ($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12))
		{
//			$button=$button."<a type='button' id='".$name."' class='btn btn-danger btn-xs' target='popup' onclick=window.open('".base_url()."index.php/HCO/delete?id=".$row->id_hco."','popup','width=300,height=100')><i class='fa fa-trash'></i>&nbsp;Delete</a><br>";
