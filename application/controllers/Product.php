...
class Product extends CI_Controller {
	...
	{
		$this->load->view('product',(array)$output);
	}
	...
	public function index()
	{
		try{
			...
			$crud->columns('id_product2','id_group','name_product','category','price','het');
			$crud->set_relation('id_group','product_group','name');
			if($this->session->userdata('id_group')==11 || $this->session->userdata('id_group')==12 || $this->session->userdata('id_group')==6 || $this->session->userdata('id_group')==13 || $this->session->userdata('id_group')==4)
			{
			}				
			else if($this->session->userdata('id_group')==19)
			{
				...
				$crud->where('product.id_group='.$id_regency);
			}
			else
			{
				$crud->unset_add();
			...
			$crud->field_type('het','integer');
			$crud->display_as('id_product2','Supplier Product ID');
			$crud->display_as('id_group','Product Group');
			$crud->display_as('name_product','Product');
			$crud->display_as('price','HNA + PPN (IDR)');
			$crud->display_as('het','HET (IDR)');
