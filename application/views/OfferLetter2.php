...
class OfferLetter2 extends CI_Controller {
	...
	public function index()
	{
		...
		{				
			...
			{				
				$query = $this->db->query("SELECT id_sc, DATE_FORMAT(created_date,'%d-%M-%Y') as created_date, event_name, facility, event_venue, nodoc2, event_institution, event_address, hcp, speciality, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date, DATE_FORMAT(event_date,'%d/%m/%Y') as event_date FROM offer_letter2 where id_ol=".$_GET['id']);
			...
			if($k==0)
			{
				...
				$query = $this->db->query("SELECT distinct DATE_FORMAT(a.created_date,'%d-%M-%Y') as created_date, DATE_FORMAT(event_start_date,'%d/%m/%Y') as event_start_date, DATE_FORMAT(event_end_date,'%d/%m/%Y') as event_end_date, event_venue, nodoc, institution_hcp FROM scientific_hcp a, scientific b where a.id_sc-b.id_sc and institution_hcp=".$_GET['institution_hcp']." and a.id_sc=".$_GET['id_sc']);
				$k = 0;
				foreach ($query->result() as $row2)
				{
					$data = array(
				...
				$speciality = "";
				$query = $this->db->query("SELECT id_speciality FROM scientific_hcp a, doctor b where a.name_hcp=b.id_doctor and institution_hcp='".$_GET['institution_hcp']."' and a.id_sc=".$_GET['id_sc']);
				foreach ($query->result() as $row2)
				{
					$speciality = $speciality.$row2->id_speciality.", ";
				}
				$speciality=rtrim($speciality);
				$speciality=rtrim($speciality, ",");

				$data = array_merge($data, array('speciality'=>$speciality));

				$facility = "";
				$query = $this->db->query("SELECT sponsor_type FROM budget_scientific a, scientific b, scientific_hcp c WHERE b.id_sc=c.id_sc and budget>0 and id_parent=c.id_sc and institution_hcp='".$_GET['institution_hcp']."' and b.id_sc='".$_GET['id_sc']."'");
				foreach ($query->result() as $row2)
				{
...
//		$result=$result."{\"id\":\"0\",\"name\":\"\"}";
	...
	public function add()
	{
		...
		$query = $this->db->query("select distinct DATE_FORMAT(b.created_date,'%d-%M-%Y') as created_date from scientific_hcp a, scientific b where a.id_sc=b.id_sc and institution_hcp=".$this->input->post('event_institution')." and id_sc=".$this->input->post('id_sc'));
		foreach ($query->result() as $row2)
		{
			$created_date=$row2->created_date;
		}
...
//------------------- Word
		...
		$section->addText('No. '.date("Y",strtotime($created_date)).'/TPI/External/'.date("m",strtotime($created_date)).'/'.$this->input->post('nodoc2'),'rStyle2','Paragraph');
		$section->addText('Kepada Yth.','rStyle','Paragraph');
		...
		$query = $this->db->query("SELECT budget, sponsor_type FROM budget_scientific where budget>0 and id_parent=".$this->input->post('id_sc'));
		foreach ($query->result() as $row2)
		{
