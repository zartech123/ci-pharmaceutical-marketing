...
<body>
    <div class="container-fluid">
		...
		<table width="100%">
			...
			<tr><td colspan="12"><center><div class="table-label" style="font-size:14px">&nbsp;<b>Trend Active Outlet</b></div></center></td></tr>
			<tr><td colspan="12">&nbsp;</td></tr>
			<tr>
...
//			minDate: date('mm/yyyy',strtotime('01/2017')),
		...
		$('#excel').click(function(e){
			var wb = XLSX.utils.table_to_book(document.getElementById('table'), {sheet:"Trend Active Outlet"});
			var pad = "00";
			var m1pad = ""+(m1+1);
			var m2pad = ""+(m2+1);
			...
			saveAs(new Blob([s2ab(wbout)],{type:"application/vnd.ms-excel;charset=utf-8"}), 'Trend Active Outlet ['+y1+'-'+pad.substring(0, pad.length - m1pad.length) + m1pad+' to '+y2+'-'+pad.substring(0, pad.length - m2pad.length) + m2pad+'].xlsx');
		});
...
function done()
{
	...
	$("#url").attr('href',url);
	$("#url").attr('download','Trend Active Outlet.png');
...
function generate(selected,selected3,selected2,response)
{
	...
	var currentTime = new Date().getTime();
			...
			/*$.ajax({
				url: "<?php echo base_url(); ?>index.php/Report3/getData?id_channel="+selected.toString()+"&id_dist="+selected3.toString()+"&id_product="+selected2.toString()+"&year1="+y1+"&year2="+y2+"&month1="+(m1+1)+"&month2="+(m2+1)+"&date="+currentTime,
					timeout:1200000,
					type: "GET",
					dataType: "text",
					headers: {
