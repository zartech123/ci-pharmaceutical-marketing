...
<body>
    <div class="container-fluid">
		<table width="100%">
			...
			<tr><td colspan="12"><center><div class="table-label" style="font-size:14px">&nbsp;<b>Trend Additional Customer</b></div></center></td></tr>
			<tr><td colspan="12">&nbsp;</td></tr>
			<tr>
				...
				<td>&nbsp;</td>
				<td><a id="url" download="filename.png" style="visibility:hidden"><i class="fa fa-bar-chart fa-border" style="font-size:21px"></i></a>&nbsp;<a id="excel" style="visibility:hidden;cursor:pointer"><i class="fa fa-text-excel-o fa-border" style="font-size:21px"></i></a></td>
			</tr>
			<tr><td colspan="12">&nbsp;</td></tr>
			<tr>
...
//		createChannel(selectedChannel,$("#id_product").val(),$("#id_dist").val());
			...
			$.ajax({
				url: "<?php echo base_url(); ?>index.php/Report6/getDistributor",
					type: "GET",
					dataType: "text",
...
//			createChannel($("#id_channel").val(),$("#id_product").val(),$("#id_dist").val());
		...
		function createChannel(selected,selected3)
		{
			var date = new Date();
			Swal.fire({
				title: '',
			...
			var content = "";
				...
				content = content + '<td style="text-align:center"><button type="button" class="btn btn-danger">Trend Additional Customer</button></td>';
...
//						content = content + '<td><button type="button" id="'+monthtotalbutton+'" class="btn btn-success" style="width:90px"></button></td><td><button id="'+monthavgbutton+'" type="button" class="btn btn-warning" style="width:95px"></button></td>';
			...
			$("#table").html(content);
//			$('#loading').text('Progress : 30%. Please wait');
//			$('.progress-bar').css('width', 30+'%').attr('aria-valuenow', 30);
			...
			$.ajax({
				url: "<?php echo base_url(); ?>index.php/Report6/getTotal3?month1="+(m1+1)+"&month2="+(m2+1)+"&id_dist="+selected3.toString()+"&year2="+y2+"&year1="+y1+"&id_channel="+selected.toString()+"&date="+currentTime,
					timeout:1200000,
					type: "GET",
					dataType: "text",
					headers: {
				...
				success: function(response)
				{
					...
					generate(selected,selected3,result2[1]);
				},
				error: function(response)
				{
					alert("xxxx");
...
//			minDate: date('mm/yyyy',strtotime('01/2017')),
		...
		$('#excel').click(function(e){
					var wb = XLSX.utils.table_to_book(document.getElementById('table'), {sheet:"Trend Additional Customer"});
					var pad = "00";
					var m1pad = ""+(m1+1);
					var m2pad = ""+(m2+1);
					...
					saveAs(new Blob([s2ab(wbout)],{type:"application/vnd.ms-excel;charset=utf-8"}), 'Trend Additional Customer ['+y1+'-'+pad.substring(0, pad.length - m1pad.length) + m1pad+' to '+y2+'-'+pad.substring(0, pad.length - m2pad.length) + m2pad+'].xlsx');
		});
...
function done()
{
	...
	$("#url").attr('href',url);
	$("#url").attr('download','Trend Additional Customer.png');

}

function generate(selected,selected3,repsonse)
{
	...
	var currentTime = new Date().getTime();
			...
			/*$.ajax({
				url: "<?php echo base_url(); ?>index.php/Report6/getData?id_channel="+selected.toString()+"&id_dist="+selected3.toString()+"&year1="+y1+"&year2="+y2+"&month1="+(m1+1)+"&month2="+(m2+1)+"&date="+currentTime,
					type: "GET",
					dataType: "text",
					headers: {
						 'Cache-Control': 'no-cache, no-store, must-revalidate', 
				...
				{*/
							var chartData = jQuery.parseJSON('' + repsonse + '');
						...
						plugins: {
							datalabels: {
							  ...
							  title: {
								display: true,
								text: "Trend Additional Customer"
							  },
							  scales: {
								yAxes: [{
									position: 'left',
